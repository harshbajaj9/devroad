...
import { PSRoleFilter } from '@/lib/lib'

const RoleSelectFilterOption = ({
  selectedRole,
  setSelectedRole
}: {
  selectedRole: { id: string; value: string } | undefined
  setSelectedRole: (role: { id: string; value: string } | undefined) => void
}) => {
  const [filterValue, setFilterValue] = useState('')

  const filteredRoles = PSRoleFilter.map(category => ({
    ...category,
    items: category.items.filter(role =>
      role.value.toLowerCase().includes(filterValue.toLowerCase())
    )
          ...
          >
            {selectedRole ? (
              <div className='flex w-28'>
                <span className='truncate'>{selectedRole.value}</span>
              </div>
            ) : (
              <div className='flex w-28'>
