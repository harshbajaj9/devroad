...
import { MoveDirectorPickerNode } from './directory-picker-node-move'
export type MoveItemType = 'ITEM' | 'SECTION' | null

const MoveSectionPicker = ({ itemType }: { itemType: MoveItemType }) => {
        ...
        >
          <PopoverTrigger asChild>
            ...
            >
              ...
              {itemType === 'SECTION' ? (
                movingItem?.id === currentPath[currentPath.length - 1]?.id ? (
                  currentPath.slice(0, -1).map((node, i) => {
                    if (i > 0)
...
export default MoveSectionPicker
